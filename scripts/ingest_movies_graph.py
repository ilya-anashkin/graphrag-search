"""Load movies JSONL into Neo4j as graph relations."""
...
DEFAULT_JSONL_PATH = "app/domains/movies/example_data/kinopoisk-top250.jsonl"


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""

    parser = argparse.ArgumentParser(description="Ingest movies JSONL into Neo4j graph")
    parser.add_argument("--file", default=DEFAULT_JSONL_PATH, help="Path to JSONL file with movies.")
...
async def ingest_graph(file_path: str, batch_size: int | None) -> None:
    ...
    progress_bar = tqdm(total=len(batches), desc="Neo4j graph ingest", unit="batch") if tqdm else None

    for batch in batches:
        ingested += await neo4j_adapter.ingest_movies(rows=batch)
    ...
    print(f"Graph ingestion done. total={total} ingested={ingested} batch_size={resolved_batch_size}")
...
def main() -> None:
    ...
    args = parse_args()
    asyncio.run(ingest_graph(file_path=args.file, batch_size=args.batch_size))
